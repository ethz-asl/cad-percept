...
CadPerceptMeshingNode::CadPerceptMeshingNode(ros::NodeHandle nh,
  ...
  if (nh_private_.param<bool>("voxel_filter/enable", true)) {
    ...
    ROS_INFO("Adding Voxel filter with size [%.2f, %.2f, %.2f].", voxel_x,
             voxel_y, voxel_z);
  ...
  sub_points_ = nh_.subscribe("points", 1,
                              &CadPerceptMeshingNode::pointCoudCallback, this);

  pub_mesh_ = nh.advertise<cgal_msgs::TriangleMeshStamped>("mesh", 1);
}

void CadPerceptMeshingNode::pointCoudCallback(
