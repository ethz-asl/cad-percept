...
CadPerceptMeshingNode::CadPerceptMeshingNode(ros::NodeHandle nh,
  ...
  if (nh_private_.param<bool>("voxel_filter/enable", true)) {
    ...
    ROS_INFO("Adding Voxel filter with size [%.2f, %.2f, %.2f].",
             voxel_x,
             voxel_y,
             voxel_z);
  ...
  preprocessing_.setNormalEstimationRadius(nh_private_.param<double>(
      "normal_estimation/radius",
      0.03));
...
void CadPerceptMeshingNode::pointCoudCallback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
  ...
  cad_percept::meshing::InputPointCloud::Ptr cloud_filtered =
      boost::make_shared<cad_percept::meshing::InputPointCloud>();
  cad_percept::meshing::InputNormals::Ptr
      normals = boost::make_shared<cad_percept::meshing::InputNormals>();
