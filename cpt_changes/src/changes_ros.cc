...
bool ChangesRos::transformModelCb(std_srvs::Empty::Request &request,
  ...
  tf_listener_.lookupTransform(map_frame_, cad_frame_, ros::Time(0),
                               transform);  // transform from cad_frame_ to map_frame_, latest
                                            // available transform, but where is cad_frame?
  ...
  mesh_model_.transform(cgal::eigenTransformationToCgalTransformation(
      transformation));  // apply transform from cad_frame_ to map_frame_ to get aligned data
  return true;
}
