...
test_Matcher::test_Matcher(ros::NodeHandle& nh, ros::NodeHandle& nh_private)
  ...
  cad_sub_ = nh_.subscribe(parameters_.cad_topic, parameters_.input_queue_size,
                           &test_Matcher::gotCAD, this);
...
void test_Matcher::gotCAD(const cgal_msgs::TriangleMeshStamped& cad_mesh_in) {
  ...
  try {
    tf_listener_.waitForTransform(parameters_.tf_map_frame, frame_id, ros::Time(0),
                                  ros::Duration(5.0));
  ...
  scan_pub_.publish(PointMatcher_ros::pointMatcherCloudToRosMsg<float>(
      ref_dp, parameters_.tf_map_frame, ros::Time::now()));
}
