  ...
  class CollisionManifoldTestNode {
   ...
   private:
    void meshCallback(const cgal_msgs::TriangleMeshStampedConstPtr& mesh) {
      ...
      ConfigProvider::Ptr cfg = std::dynamic_pointer_cast<ConfigProvider>(
          std::make_shared<RosConfigProvider>(nh_private_));

      // Create construction strategy
      auto construction_strategy = std::make_shared<offset_surface::MeshDomainStrategy>(cfg);
