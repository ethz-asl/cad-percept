...
void Deviations::planarSegmentationCGAL(const PointCloud &cloud, std::vector<reconstructed_plane> *rec_planes, PointCloud *remaining_cloud) const {
...
void Deviations::runShapeDetection(const PointCloud &cloud, std::vector<reconstructed_plane> *rec_planes, PointCloud *remaining_cloud) const {
  ...
  */                                                                                              
  ...
  for (size_t i = 0; i<1; i++) { // choose number of iterations here
    ...
    if (coverage > best_coverage) {
      ...
      typename ShapeDetection::Point_index_range unassigned_pts_range = shape_detection.indices_of_unassigned_points();
      typename ShapeDetection::Point_index_range::iterator pit = unassigned_pts_range.begin();
      while (pit != unassigned_pts_range.end()) {
        pcl::PointXYZ unassigned_point = pcl::PointXYZ(points[*pit].first.x(), points[*pit].first.y(), points[*pit].first.z());
...
void Deviations::associatePlane(cgal::MeshModel &mesh_model, const reconstructed_plane &rec_plane, int *id, double *match_score, bool *success) {
  ...
  *success = false;
  ...
  for (association_bimap::right_const_iterator i = bimap.right.begin(); i != bimap.right.end(); i = bimap.right.upper_bound(i->first)) {
    ...
    if (ratio > params.assocAreaRatioUpperLimit || (plane_map[i->first].area < params.assocAreaLowerLimitThreshold && ratio < params.assocAreaRatioLowerLimit)) {
      continue;
    }
    ...
    for (auto point : cloud.points) {
      ...
      for (auto plane : boost::make_iterator_range(bimap.right.equal_range(i->first))) {
        d = std::min(d, CGAL::to_double(CGAL::squared_distance(cgal::Point(point.x, point.y, point.z), mesh_model.getTriangleFromID(plane.second))));
      }
    ...
    //std::cout << "P.C. Normal: " << rec_plane.pc_normal << ", Ref. plane normal: " << plane_map[i->first].normal << std::endl; 
    ...
    match_score_new = params.distWeight * avg_dist + params.minDistWeight * min_dist + params.distanceScoreWeight * distance_score + params.angleWeight * angle;
    ...
    if (match_score_new > params.matchScoreUpperLimit) {
      //std::cout << "match_score is: " << match_score_new << ", but threshold at: " << params.matchScoreUpperLimit << std::endl;
      continue;
    }
...
void Deviations::findBestPlaneAssociation(std::vector<reconstructed_plane> cloud_vector, cgal::MeshModel &mesh_model, std::vector<reconstructed_plane> *remaining_plane_cloud_vector) {
...
void Deviations::findPlaneDeviation(std::unordered_map<int, transformation> *current_transformation_map, bool size_check) {
  for (Umiterator umit = plane_map.begin(); umit != plane_map.end(); ++umit) {
    if (umit->second.associated == true) {
      ...
      if (size_check) {
        /**
         *  Better check horizontal and vertical distance ratio here, so that we can analyze in complete x,y-direction (if horizontal ratio is right),
         *  and additionally z-direction (if vertical ratio is right)
         */
        ...
        if (pc_area < 0 * umit->second.area || pc_area > 5 * umit->second.area) { // better use [0.9, 1.1], but keep in mind that current lidar does not have full view
          std::cout << "Size wrong, area: " << pc_area << "/ " << umit->second.area << std::endl;
          continue;
        }
        // horizontal/vertical size check
        double modelHorDim, modelVertDim, recHorDim, recVertDim;
        cpt_utils::bboxDiameters(umit->second.bbox, &modelHorDim, &modelVertDim);
        cpt_utils::bboxDiameters(umit->second.rec_plane.pointcloud, &recHorDim, &recVertDim);
