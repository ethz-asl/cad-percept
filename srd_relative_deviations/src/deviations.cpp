...
void Deviations::detectChanges(std::vector<reconstructed_plane> *rec_planes_publish,
                               const PointCloud &reading_cloud, PointCloud *icp_cloud,
                               std::ifstream &ifs_icp_config, std::ifstream &ifs_normal_filter,
                               std::ifstream &ifs_selective_icp_config,
  ...
  if (references.empty() == 1) {
    ...
    selectiveICP(ifs_selective_icp_config, ifs_normal_filter, no_of_points, P, reading_cloud,
                 references, &pointcloud_out);
  }
...
void Deviations::extractReferenceFacets(const int no_of_points, cgal::Polyhedron &P,
                                        std::unordered_set<int> &references,
  ...
  // not sure if it makes sense to use whole colinear plane or just the given facets with a very
  // small filter ratio
  // -> balance between how many points we can match from reading vs. misorientation due to errors
  // in colinear facets find better filter
  ...
  // with trimmed dist outlier filter ratio 0.3 , this ratio can be as small as possible, but
  // depends on how many associations we have with reference vs. total associations (we only want to
  // keep the correct associations)
...
void Deviations::selectiveICP(std::ifstream &ifs_icp_config, std::ifstream &ifs_normal_filter,
                              const int no_of_points, cgal::Polyhedron &P,
                              const PointCloud &reading_cloud, std::unordered_set<int> &references,
                              PointCloud *pointcloud_out) {
  PointCloud icp_pointcloud;
...
void Deviations::ICP(std::ifstream &ifs_icp_config, std::ifstream &ifs_normal_filter,
                     const PointCloud &reading_cloud, PointCloud *pointcloud_out) {
  // convert point clouds to DP
  DP dppointcloud = cpt_utils::pointCloudToDP(reading_cloud);
...
void Deviations::planarSegmentationPCL(const PointCloud &cloud_in,
                                       std::vector<reconstructed_plane> *rec_planes) const {
  // PCL solution
  ...
  while (cloud_filtered->points.size() > 0.01 * nr_points) {
    ...
    /*
    std::stringstream ss;
    ss << "/home/julian/megabot_ws/src/cad-percept/srd_relative_deviations/resources/plane_" << i <<
    ".pcd"; writer.write<pcl::PointXYZ> (ss.str(), *cloud_p, false);
    */
...
void Deviations::runShapeDetection(const PointCloud &cloud,
  ...
  */
  ...
  while (it != shapes.end()) {
    ...
    std::cout << "Kernel::Plane_3: " << static_cast<cgal::ShapeKernel::Plane_3>(*plane)
              << std::endl;
...
void Deviations::associatePlane(cgal::MeshModel &mesh_model, const PointCloud &cloud, int *id,
                                double *match_score) {
  *match_score = 0;
  for (association_bimap::right_const_iterator i = bimap.right.begin(); i != bimap.right.end();
    ...
    cgal::FT d = 0;
    for (auto point : cloud.points) {
      d += CGAL::squared_distance(cgal::Point(point.x, point.y, point.z),
    ...
    if (match_score_new < *match_score || *match_score == 0) {
      if (match_score_new == plane_map[i->first].match_score) {
        std::cout << "Match score new is: " << match_score_new
                  << ", map score is: " << plane_map[i->first].match_score << std::endl;
        std::cerr << "Pointcloud with same match score. Keeping preceding result." << std::endl;
                 ...
                 plane_map[i->first].match_score == 0) {
...
void Deviations::findBestPlaneAssociation(
    const std::vector<reconstructed_plane> &cloud_vector, cgal::MeshModel &mesh_model,
    std::vector<reconstructed_plane> *remaining_cloud_vector) {
  // create queue
...
double Deviations::getICPError(const PointCloud &aligned_pc,
  ...
  for (auto point : aligned_pc) {
    ...
    double squared_distance =
        reference_mesh.squaredDistance(cgal::Point(point.x, point.y, point.z));
    if (references.find(reference_mesh.getFacetIndex(ppid.second)) != references.end() &&
