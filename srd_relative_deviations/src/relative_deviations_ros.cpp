...
void RelativeDeviations::readingCallback(cgal::PointCloud &reading_pc) {
  ...
  std::ifstream ifs_icp_config(
      nh_private_.param<std::string>("icp_configuration_file", "fail").c_str());
  std::ifstream ifs_selective_icp_config(
      nh_private_.param<std::string>("selective_icp_configuration_file", "fail").c_str());
  std::ifstream ifs_normal_filter(
      nh_private_.param<std::string>("normal_filter_file", "fail").c_str());
  std::vector<reconstructed_plane> remaining_cloud_vector;
  std::unordered_map<int, transformation> transformation_map;
  deviations.detectChanges(&rec_planes, reading_pc, &icp_cloud, ifs_icp_config, ifs_normal_filter,
                           ifs_selective_icp_config, &remaining_cloud_vector, &transformation_map);
...
void RelativeDeviations::publishAssociations(
    const cgal::MeshModel &model, std::unordered_map<int, polyhedron_plane> &plane_map,
    const std::vector<reconstructed_plane> &remaining_cloud_vector) {
  ColoredPointCloud pointcloud_rgb;
  ColoredPointCloud pointcloud_plane_rgb;
  ...
  for (Umiterator umit = plane_map.begin(); umit != plane_map.end(); ++umit) {
    if (umit->second.match_score != 0) {
      ...
      for (auto point : umit->second.rec_plane.pointcloud) {
        ...
        p_from.y = point.y;
        p_from.z = point.z;
        cgal::Point p = cpt_utils::closestPointOnPlane(umit->second.plane,
                                                       cgal::Point(point.x, point.y, point.z));
        p_to.x = p.x();
        p_to.y = p.y();
        p_to.z = p.z();
...
void RelativeDeviations::publishDeviations(
  ...
  // - overwrite color of associated planes/triangles with deviation gradient based on e.g. distance
  // score, because distance score includes both angle deviation and distance deviation
  ...
  for (Umiterator umit = plane_map.begin(); umit != plane_map.end(); ++umit) {
    if (umit->second.match_score != 0) {
      ...
      // std::unordered_map<int,transformation>::const_iterator it =
      // transformation_map.find(umit->first);
