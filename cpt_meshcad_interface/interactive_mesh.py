...
def make6DofMarker(fixed, interaction_mode, position, show_6dof=False):
    ...
    if not interaction_mode == InteractiveMarkerControl.NONE:
        control_modes_dict = {
            ...
            InteractiveMarkerControl.MOVE_ROTATE_3D: "MAP ALIGNMENT"} # MOVE_ROTATE_3D
    ...
    if show_6dof:
        ...
        # move block freely without this
        #control = InteractiveMarkerControl()
...
def initMenu():
    ...
    h_fifth_entry = menu_handler.insert("Load CAD",
                                        callback=loadCADCb)


if __name__ == "__main__":
    rospy.init_node("interactive_mesh_marker_controls")
    br = TransformBroadcaster()

    # Create a timer to update the published transforms
    rospy.Timer(rospy.Duration(0.01), frameCallback)

    server = InteractiveMarkerServer("interactive_mesh_marker_controls")
