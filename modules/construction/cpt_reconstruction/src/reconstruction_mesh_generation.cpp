  ...
  if (counter_ >= 400 && (counter_ % 400 == 0)){
    ...
    for (unsigned i = 0; i < clouds_.size(); i++){
      if (clouds_[i]->size() > 500){
        ...
        //Source: https://stackoverflow.com/questions/49688940/point-cloud-library-rotation-of-axis-alligned-bounding-box
...
void MeshGeneration::purgeBuffer(){
  ...
  for (int i = 0; i < clouds_.size(); i++){
    if (std::find(blocked_idx.begin(), blocked_idx.end(), i) != blocked_idx.end()) {
      continue;
    }
    std::vector<int> fusing_temp;
    fusing_temp.push_back(i);
    for (int j = i + 1; j < clouds_.size(); j++){
      if (std::find(blocked_idx.begin(), blocked_idx.end(), j) != blocked_idx.end()) {
        continue;
      }
      double diff1 = (ransac_normals_[i] - ransac_normals_[j]).lpNorm<Eigen::Infinity>();
      double diff2 = (ransac_normals_[i] + ransac_normals_[j]).lpNorm<Eigen::Infinity>();
  ...
  for (int i = 0; i < fusing.size(); i++){
    ...
    if (fused_point_cloud->size() > 500) {
      ...
      pcl::search::KdTree<pcl::PointXYZ>::Ptr fused_kd_tree(new pcl::search::KdTree<pcl::PointXYZ>());
...
if (existing_shape){
  if (matching_idx.size() == 1){
    ...
    for (unsigned r = 0; r < matching_idx.size(); r++){
      ...
      ransac_normals_.erase(ransac_normals_.begin() + matching_idx.at(r) - idx_corr);
      idx_corr++;
    }
  }
}
