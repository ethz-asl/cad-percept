...
void MeshGeneration::messageCallback(
  ...
  std::vector<int> meshing_classes;
  ...
  for (int i = 0; i < msg.id.size(); i++) {
    ...
    if (valid_class) {
      ...
      meshing_clouds.push_back(cur_cloud);
      if (cur_class == Semantics::WALL) {
        meshing_classes.push_back(Semantics::WALL);

      } else if (cur_class == Semantics::FLOOR) {
        meshing_classes.push_back(Semantics::FLOOR);

      } else if (cur_class == Semantics::CEILING) {
        meshing_classes.push_back(Semantics::CEILING);
      }
      computePlanarConvexHull(cur_cloud, mesh);
      combineMeshes(mesh, mesh_detected);
    }
  }
  pcl::io::savePLYFile("/home/philipp/Schreibtisch/mesh_walls.ply",
                       mesh_detected);

  pcl::PolygonMesh mesh_model;
  pcl::io::loadPolygonFilePLY(
      "/home/philipp/Schreibtisch/data/CLA_MissingParts_3.ply", mesh_model);

  pcl::PointCloud<pcl::PointXYZ>::Ptr vertices_model_only(
      new pcl::PointCloud<pcl::PointXYZ>);
  pcl::fromPCLPointCloud2(mesh_model.cloud, *vertices_model_only);
  std::vector<::pcl::Vertices> faces_model_only = mesh_model.polygons;

  combineMeshes(mesh_detected, mesh_model);

  std::vector<::pcl::Vertices> faces_model = mesh_model.polygons;
  pcl::PointCloud<pcl::PointXYZ>::Ptr vertices_model(
      new pcl::PointCloud<pcl::PointXYZ>);
  pcl::fromPCLPointCloud2(mesh_model.cloud, *vertices_model);
  ...
  for (int i = 0; i < meshing_clouds.size(); i++) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr element_points = meshing_clouds.at(i);
  ...
  for (int i = 0; i < center_estimates.size(); i++) {
    ...
    ROS_INFO("a1_max %d", a1_max);
    ROS_INFO("a2_max %d", a2_max);
    ROS_INFO("b1_max %d", b1_max);
    ROS_INFO("b2_max %d", b2_max);
    ROS_INFO("c1_max %d", c1_max);
    ROS_INFO("c2_max %d", c2_max);
...
bool MeshGeneration::checkShapeConstraints(int sem_class,
  ...
  if (sem_class == Semantics::WALL) {
    if (std::fabs(normal.z()) < 0.1) {
      return true;
    } else {
      return false;
    }
  } else if (sem_class == Semantics::FLOOR || sem_class == Semantics::CEILING) {
...
void MeshGeneration::flagDuplicatedPlanes(
    std::vector<::pcl::Vertices> &faces_model,
    std::vector<Eigen::Vector3d> &plane_normals, std::vector<double> &plane_d,
    std::vector<double> &area, std::vector<int> &duplicated_faces,
    double min_area) {
  for (int i = 0; i < faces_model.size(); i++) {
...
void MeshGeneration::selectMainCandidateFaces(
    ...
    std::vector<double> &plane_d, double min_area, bool plane_flipping) {
...
void MeshGeneration::selectOrthoCandidateFaces(
    std::vector<::pcl::Vertices> &faces_model,
    std::vector<int> &duplicated_faces,
    pcl::PointCloud<pcl::PointXYZ>::Ptr vertices_model,
    pcl::PointCloud<pcl::PointXYZ>::Ptr corners_side,
    ...
    std::vector<double> &plane_d,
    std::vector<int> &candidate_faces_ortho_horizontal,
    std::vector<int> &candidate_faces_ortho_vertical, double min_area) {
  for (int i = 0; i < faces_model.size(); i++) {
    // Ignore duplicated planes or very small areas
    if (std::find(duplicated_faces.begin(), duplicated_faces.end(), i) !=
            duplicated_faces.end() ||
