#ifndef CPT_RECONSTRUCTION_INCLUDE_CPT_RECONSTRUCTION_RECONSTRUCTION_CUSTOM_CYLINDER_H_
#define CPT_RECONSTRUCTION_INCLUDE_CPT_RECONSTRUCTION_RECONSTRUCTION_CUSTOM_CYLINDER_H_

// Source:
// https://github.com/CGAL/cgal/blob/master/Shape_detection/include/CGAL/Shape_detection/Efficient_RANSAC/Cylinder.h
// Fully copied from source and restricted radius to 0.8
...
class Custom_Cylinder : public Shape_base<Traits> {
 ...
 public:
  Custom_Cylinder() : Shape_base<Traits>() {}

  /*!
    Axis of the cylinder.
   */
  Line_3 axis() const { return m_axis; }
  ...
  /*!
    Helper function to write axis and radius of the cylinder and number of
    assigned points into a string.
   */
  std::string info() const {
    ...
    sstr << "Type: cylinder center: (" << this->get_x(c) << ", "
         << this->get_y(c) << ", " << this->get_z(c) << ") axis: ("
         << this->get_x(a) << ", " << this->get_y(a) << ", " << this->get_z(a)
         << ") radius:" << m_radius << " #Pts: " << this->m_indices.size();
 ...
 protected:
  ...
  virtual void create_shape(const std::vector<std::size_t> &indices) {
    ...
    Vector_3 xDir =
        this->sum_vectors(n1, this->scale(axis, -this->scalar_pdct(n1, axis)));
  ...
  virtual void parameters(const std::vector<std::size_t> &indices,
                          ...
                          FT &cluster_epsilon, FT min[2], FT max[2]) const {
    Vector_3 d1 =
        this->constr_vec(ORIGIN, this->constr_pt(FT(0), FT(0), FT(1)));
    Vector_3 a = this->constr_vec(m_axis);
    a = this->scale(a, (FT)1.0 / CGAL::sqrt(this->sqlen(a)));
  ...
  FT cos_to_normal(const Point_3 &p, const Vector_3 &n) const {
    ...
    FT length = CGAL::sqrt(this->sqlen(v));
    if (length == 0) return (FT)1.0;

    v = this->scale(v, (FT)1.0 / length);
    return CGAL::abs(this->scalar_pdct(v, n));
  }

  virtual std::size_t minimum_sample_size() const { return 2; }
...
#endif  // CPT_RECONSTRUCTION_INCLUDE_CPT_RECONSTRUCTION_RECONSTRUCTION_CUSTOM_CYLINDER_H_
