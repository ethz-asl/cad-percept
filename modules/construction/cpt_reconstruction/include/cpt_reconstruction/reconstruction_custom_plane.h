#ifndef CPT_RECONSTRUCTION_INCLUDE_CPT_RECONSTRUCTION_RECONSTRUCTION_CUSTOM_PLANE_H_
#define CPT_RECONSTRUCTION_INCLUDE_CPT_RECONSTRUCTION_RECONSTRUCTION_CUSTOM_PLANE_H_
// Source:
// https://github.com/CGAL/cgal/blob/master/Shape_detection/include/CGAL/Shape_detection/Efficient_RANSAC/Plane.h
// Fully copied from source and changed internal parameters
...
/*!
  ...
  \brief Plane implements Shape_base. The plane is represented by the normal
  vector and the distance to the origin.
...
class Custom_Plane : public Shape_base<Traits> {
 public:
  /// \cond SKIP_IN_MANUAL
  ...
  typedef typename Traits::Plane_3
      Plane_3;  ///< %Plane type for the conversion operator.

 public:
  Custom_Plane() : Shape_base<Traits>(), random_(CGAL::Random(0)) {}

  /*!
    Conversion operator to `Plane_3` type.
   */
  operator Plane_3() const {
    return Plane_3(this->get_x(m_normal), this->get_y(m_normal),
                   this->get_z(m_normal), m_d);
  }

  /*!
    Normal vector of the plane.
   */
  Vector_3 plane_normal() const { return m_normal; }

  /*!
    Signed distance from the origin.
   */
  FT d() const { return m_d; }
  ...
  FT squared_distance(const Point_3 &p) const {
    FT d =
        this->scalar_pdct(this->constr_vec(p, m_point_on_primitive), m_normal);
    return d * d;
  }
   ...
   */
  Point_3 projection(const Point_3 &p) const { return to_3d(to_2d(p)); }
  ...
  */
  void update(const Plane_3 &p) {
    ...
    m_normal = p.orthogonal_vector() /
               std::sqrt(p.orthogonal_vector() * p.orthogonal_vector());
 ...
 protected:
  ...
  virtual void create_shape(const std::vector<std::size_t> &indices) {
    ...
    m_normal =
        this->cross_pdct(this->constr_vec(p2, p1), this->constr_vec(p3, p1));
    ...
    for (std::size_t i = 0; i < 3; i++) {
      ...
      m_base1 =
          this->scale(m_base1, ((FT)1.0 / CGAL::sqrt(this->sqlen(m_base1))));

      m_base2 = this->cross_pdct(m_base1, m_normal);
      m_base2 =
          this->scale(m_base2, ((FT)1.0 / CGAL::sqrt(this->sqlen(m_base2))));
    }
  ...
  virtual void parameters(const std::vector<std::size_t> &indices,
                          std::vector<std::pair<FT, FT> > &parameterSpace, FT &,
                          FT min[2], FT max[2]) const {
    // Transform first point before to initialize min/max
    Vector_3 p =
        this->constr_vec(m_point_on_primitive, this->point(indices[0]));
  ...
  virtual void cos_to_normal(const std::vector<std::size_t> &indices,
    ...
    for (std::size_t i = 0; i < indices.size(); i++) {
      angles[i] =
          CGAL::abs(this->scalar_pdct(this->normal(indices[i]), m_normal));
    }
  }
  ...
  virtual std::size_t minimum_sample_size() const { return 3; }

  virtual bool supports_connected_component() const { return true; }
...
#endif  // CPT_RECONSTRUCTION_INCLUDE_CPT_RECONSTRUCTION_RECONSTRUCTION_CUSTOM_PLANE_H_
