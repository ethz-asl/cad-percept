...
#ifndef CPT_CHOMP_PLANNING_CHOMP_OPTIMIZER_H
#define CPT_CHOMP_PLANNING_CHOMP_OPTIMIZER_H
...
struct ChompTrajectory {
 public:
  ...
  Eigen::VectorXd getAcceleration(int row) const {
    if (row == 0) {
      // Forward differentiation at first points
      return N * N * (trajectory.row(row + 2) - 2 * trajectory.row(row + 1) + trajectory.row(row));
    } else if (row == trajectory.rows() - 1) {
      // Backwards at last point
      return N * N * (trajectory.row(row - 2) - 2 * trajectory.row(row - 1) + trajectory.row(row));
    } else {
      // Middle everywhere else
      return N * N * (trajectory.row(row + 1) - 2 * trajectory.row(row) + trajectory.row(row - 1));
    }
  }
};
...
class ChompCostFunc {
 public:
  ...
  virtual void getGradient(const ChompTrajectory& traj, Eigen::MatrixXd* gradient) const {
...
class SmoothnessFunc : public ChompCostFunc {
 public:
  ...
  virtual bool isInitialized() const { return A_.rows() != 0 && A_.cols() != 0; }
...
class CollisionFunc : public ChompCostFunc {
 public:
  typedef std::function<double(const Eigen::VectorXd& position)> DistanceFunctionType;
  ...
  void setDistanceFunction(const DistanceFunctionType& function) { distance_function_ = function; }
  void setDistanceGradientFunction(const DistanceGradientFunctionType& function) {
    distance_gradient_function_ = function;
  }
  ...
  double getDistance(const Eigen::VectorXd& pos) const { return distance_function_(pos); }

  Eigen::VectorXd getDistanceGradient(const Eigen::VectorXd& pos) const {
    return distance_gradient_function_(pos);
  }

 protected:
  DistanceFunctionType distance_function_;
  DistanceGradientFunctionType distance_gradient_function_;
};
...
class ChompOptimizer {
 public:
  ...
  void setDistanceFunction(const CollisionFunc::DistanceFunctionType& function) {
    f_collision_.setDistanceFunction(function);
  }
  void setDistanceFunctions(const CollisionFunc::DistanceFunctionType& function,
                            const CollisionFunc::DistanceGradientFunctionType& function_g) {
    f_collision_.setDistanceFunction(function);
    f_collision_.setDistanceGradientFunction(function_g);
  }

  // Convenience functions.
  void createTrajectory(const Eigen::VectorXd& start, const Eigen::VectorXd& goal, int N,
                        ChompTrajectory* traj) const;
  void getGradient(const ChompTrajectory& traj, Eigen::MatrixXd* gradient) const;
 ...
 private:
  ...
  // horrible ros-direct-publisher to figure out
  // whats going on here
...
#endif  // CPT_CHOMP_PLANNING_CHOMP_OPTIMIZER_H
