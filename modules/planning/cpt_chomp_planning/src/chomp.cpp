...
double SmoothnessFunc::getCost(const ChompTrajectory& traj) const {
  ...
  double cost =
      (traj.N - 1) * (0.5 * traj.trajectory.block(1, 0, traj.N - 2, params_.D).transpose() * A_ *
                          traj.trajectory.block(1, 0, traj.N - 2, params_.D) +
...
void SmoothnessFunc::getGradient(const ChompTrajectory& traj, Eigen::MatrixXd* gradient) const {
  CHECK_NOTNULL(gradient);
  *gradient = (traj.N - 1) * (A_ * traj.trajectory.block(1, 0, traj.N - 2, params_.D) + b_);
}
...
double CollisionFunc::getCost(const ChompTrajectory& traj) const {
  ...
  for (int i = 1; i < traj.trajectory.rows() - 1; ++i) {
    ...
    double cost = 0.5 / (traj.N - 1) * velocity.norm() * (potential + last_potential);
...
void CollisionFunc::getGradient(const ChompTrajectory& traj, Eigen::MatrixXd* gradient) const {
  // Gradient timeeee!
  ...
  for (int i = 1; i < traj.trajectory.rows() - 1; ++i) {
    ...
    for (int j = 0; j < params_.D; ++j) {
      ...
      double left_distance =
          potentialFunction(distance_function_(traj.trajectory.row(i) - increment.transpose()));
      double right_distance =
          potentialFunction(distance_function_(traj.trajectory.row(i) + increment.transpose()));
      double dim_gradient = (right_distance - left_distance) / (2.0 * params_.map_resolution);
...
void ChompOptimizer::setup() {
  ros::NodeHandle nh;
  pub_marker_ = nh.advertise<visualization_msgs::MarkerArray>("chomp_marker", 0);
}

void ChompOptimizer::createTrajectory(const Eigen::VectorXd& start, const Eigen::VectorXd& goal,
                                      int N, ChompTrajectory* traj) const {
  CHECK_NOTNULL(traj);
...
// Returns cost.
void ChompOptimizer::getGradient(const ChompTrajectory& traj, Eigen::MatrixXd* gradient) const {
  CHECK_NOTNULL(gradient);
  ...
  mav_trajectory_generation::timing::Timer timer_grad_smooth("chomp/gradient_smooth");
  f_smooth_.getGradient(traj, &grad_smooth);
  timer_grad_smooth.Stop();

  if (f_collision_.isInitialized()) {
    mav_trajectory_generation::timing::Timer timer_grad_coll("chomp/gradient_coll");

    f_collision_.getGradient(traj, &grad_collision);
    timer_grad_coll.Stop();
  ...
  *gradient = params_.w_smooth * grad_smooth + params_.w_collision * grad_collision;
}
...
double ChompOptimizer::getCost(const ChompTrajectory& traj) const {
  ...
  mav_trajectory_generation::timing::Timer timer_cost_smooth("chomp/cost_smooth");
  double cost_smooth = f_smooth_.getCost(traj);
  timer_cost_smooth.Stop();

  double cost_collision = 0.0;
  if (f_collision_.isInitialized()) {
    mav_trajectory_generation::timing::Timer timer_cost_coll("chomp/cost_collision");
...
bool ChompOptimizer::doGradientDescent(ChompTrajectory* traj) {
  ...
  for (i = 0; i < params_.max_iter; ++i) {
    ...
    Eigen::MatrixXd traj_increment(traj->trajectory.rows(), traj->trajectory.cols());

    traj_increment.setZero();
    // M \ grad

    traj_increment.block(1, 0, traj->N - 2, params_.D) = M_.ldlt().solve(gradient);
    ...
    if (params_.verbose && i % 1 == 0) {
      std::cout << "[GD] i: " << i << " step size: " << step_size << " cost: " << cost
    ...
    if (i > 5) {
      ...
      if (rel_error < params_.rel_tol) {
        if (params_.verbose) {
          std::cout << "[GD][Finished][Tol] Iter " << i << " finished with cost " << cost
                    << " with rel_error " << rel_error << std::endl;
        }
        break;
      }
      prev_cost = cost;
