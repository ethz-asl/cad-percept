...
class RMPFreeSpaceNode {
 public:
  RMPFreeSpaceNode(ros::NodeHandle nh) : nh_(nh) {
    ...
    server_.setCallback(boost::bind(&RMPFreeSpaceNode::config_callback, this, _1, _2));
  ...
  void callback() {
    ...
    int num=0;
    double z = 1.5;
    for (double x = -1.5; x <= 1.5; x += 0.125) {
      for (double y = -1.5; y <= 1.5; y += 0.125) {
