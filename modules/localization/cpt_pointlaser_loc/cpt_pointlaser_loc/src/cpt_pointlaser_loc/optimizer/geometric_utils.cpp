...
Eigen::Matrix<double, 6, 1> getIntersectionPlaneImplementation(
  ...
  cad_percept::cgal::Ray query_ray =
      cad_percept::cpt_utils::buildRayFromPose(sensor_pose);
  cad_percept::cgal::Intersection i = model->getIntersection(query_ray);
  Eigen::Matrix<double, 6, 1> response;
  response.head<3>() =
      Eigen::Vector3d(i.intersected_point.x(), i.intersected_point.y(),
                      i.intersected_point.z());
  response.tail<3>() = Eigen::Vector3d(
      i.surface_normal.x(), i.surface_normal.y(), i.surface_normal.z());
  return response;
}
