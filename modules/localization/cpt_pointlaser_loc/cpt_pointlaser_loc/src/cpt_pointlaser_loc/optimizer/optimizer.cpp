...
Eigen::Vector3d LocalizationOptimizer::addRelativeMeasurement(
  ...
  if (fix_retrieved_planes_) {
    ...
    cad_percept::cgal::Intersection plane =
        architect_model_->getIntersection(query_ray);
    plane_support = std::make_shared<EVector3>(Eigen::Vector3d(
        plane.intersected_point.x(), plane.intersected_point.y(),
        plane.intersected_point.z()));
    plane_normal = std::make_shared<EVector3>(
        Eigen::Vector3d(plane.surface_normal.x(), plane.surface_normal.y(),
                        plane.surface_normal.z()));
  } else {
    // make a constant from the architecture model
    auto model =
        gtsam::Expression<cad_percept::cgal::MeshModel::Ptr>(architect_model_);
