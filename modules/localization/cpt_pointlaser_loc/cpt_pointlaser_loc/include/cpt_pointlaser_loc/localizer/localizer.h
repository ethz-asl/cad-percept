...
class PointLaserLocalizer {
 public:
  ...
  ///
  /// \param marker_to_armbase          Pose between the marker and the arm
  ///                                   base.
  /// \param initial_pose               Pose between the arm base and the Kinova
  ///                                   link.
  /// \param laser_a_offset             Pose between the Kinova link and the
  ///                                   point laser A.
  /// \param laser_b_offset             Pose between the Kinova link and the
  ///                                   point laser B.
  /// \param laser_c_offset             Pose between the Kinova link and the
  ///                                   point laser C.
  /// \param endeffector_offset         Pose between the Kinova link and the
  ///                                   Kinova end effector.
  /// \param arm_base_to_base           Pose between the arm base and the base.
  /// \param fix_cad_planes             If True, the planes retrieved by the
  ///                                   optimizer are fixed.
  //  \param add_initial_pose_prior     If True, a factor for the prior of the
  ///                                   model offset is added to the
  ///                                   optimization graph.
  /// \param only_optimize_translation  If True, only optimizes translation.
  /// \return True if localization can be successfully performed, False
  //          otherwise.
  bool localize(const kindr::minimal::QuatTransformation& marker_to_armbase,
                ...
                bool fix_cad_planes = false,
                bool add_initial_pose_prior = false,
