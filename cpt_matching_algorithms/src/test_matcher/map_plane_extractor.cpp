...
MapPlanes::MapPlanes(
  ...
  for (auto plane_inlier : extracted_map_inliers) {
    ...
    if ((Eigen::Vector3f(norm_point.x, norm_point.y, norm_point.z) -
         point_in_map)
            .dot(Eigen::Vector3f(norm_point.normal_x, norm_point.normal_y,
  ...
  std::cout
      << "Loaded normals and centroids of planes, find boundaries of planes ..."
      << std::endl;
  ...
  for (auto plane_inlier : extracted_map_inliers) {
    ...
    plane_boundaries(0, i) = *std::min_element(dim_x_of_plane.begin(),
                                               dim_x_of_plane.end()); // x_min
    plane_boundaries(1, i) = *std::max_element(dim_x_of_plane.begin(),
                                               dim_x_of_plane.end()); // x_max
    plane_boundaries(2, i) = *std::min_element(dim_y_of_plane.begin(),
                                               dim_y_of_plane.end()); // y_min
    plane_boundaries(3, i) = *std::max_element(dim_y_of_plane.begin(),
                                               dim_y_of_plane.end()); // y_max
    plane_boundaries(4, i) = *std::min_element(dim_z_of_plane.begin(),
                                               dim_z_of_plane.end()); // z_min
    plane_boundaries(5, i) = *std::max_element(dim_z_of_plane.begin(),
...
void MapPlanes::dispAllPlanes() {
  ...
  for (auto plane : plane_centroid_with_normals_) {
    std::cout << "plane nr. " << plane_nr << " x: " << plane.x
              << " y: " << plane.y << "z : " << plane.z << std::endl;
    std::cout << "normal x: " << plane.normal_x
              << " normal y: " << plane.normal_y
              << " normal z: " << plane.normal_z << std::endl;
    std::cout << "boundaries: xmin: " << plane_boundaries(0, plane_nr)
              << " boundaries: xmax: " << plane_boundaries(1, plane_nr)
              << std::endl;
    std::cout << "boundaries: ymin: " << plane_boundaries(2, plane_nr)
              << " boundaries: ymax: " << plane_boundaries(3, plane_nr)
              << std::endl;
    std::cout << "boundaries: zmin: " << plane_boundaries(4, plane_nr)
              << " boundaries: zmax: " << plane_boundaries(5, plane_nr)
              << std::endl;
    plane_nr++;
  }
};
...
void MapPlanes::saveToYamlFile(std::string file_name) {
  ...
  for (int map_plane_nr = 0; map_plane_nr < plane_centroid_with_normals_.size();
       ++map_plane_nr) {
    actual_plane_name = plane_name + std::to_string(map_plane_nr);
    node[actual_plane_name].push_back(
        plane_centroid_with_normals_.points[map_plane_nr].x);
    node[actual_plane_name].push_back(
        plane_centroid_with_normals_.points[map_plane_nr].y);
    node[actual_plane_name].push_back(
        plane_centroid_with_normals_.points[map_plane_nr].z);
    node[actual_plane_name].push_back(
        plane_centroid_with_normals_.points[map_plane_nr].normal_x);
    node[actual_plane_name].push_back(
        plane_centroid_with_normals_.points[map_plane_nr].normal_y);
    node[actual_plane_name].push_back(
        plane_centroid_with_normals_.points[map_plane_nr].normal_z);
...
void MapPlanes::loadFromYamlFile(std::string file_name) {
  ...
  plane_boundaries =
      Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>::Zero(6, map_size);
  std::string actual_plane_name;
  std::string plane_name = "map_plane_nr_";
